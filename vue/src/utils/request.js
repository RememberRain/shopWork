...
/* 第三部分: 请求拦截器(发送请求前要干的事情) */

service.interceptors.request.use(
    config => {
      //1.检查是否需要添加token (有些请求不需要身份认证)
      const isToken = config.headers?.isToken !== false
      const isRepeatSubmit = config.headers?.repeatSubmit !== false

      //如果有token并且这个请求需要token
      if (getToken() && isToken) {
        //在请求头里面加上token
        ...
        if (sessionObj) {
          ...
          //检查是否重复提交
            if (s_data === requestObj.data && (requestObj.time - s_time) < interval && s_url === requestObj.url ) {
                const message = '操作过于频繁，请稍后再试'
                ElMessage.error(message)
                return Promise.reject(new Error(message)) //
            }
        }

        //不是重复提交, 就保存这次请求记录(用于下次比较)
    ...
    error => {
      ...
      //2.错误信息翻译
        const errMap = {
            ...
            'timeout': '系统接口请求超时',
            'Request failed with status code': '系统接口' + (message ? message.substring(message.length - 3) : '状态') + '异常'
        }

      //3.遍历错误映射表, 匹配错误信息
      Object.keys(errMap).forEach(key => {
        //如果错误信息包含关键词, 就替换成翻译信息
